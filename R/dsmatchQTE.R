#' Double Score Matching Estimator for Quantile Treatment Effect.
#'
#' \code{dsmatchQTE} applys matching algorithm to estimate quantile
...
#'
#' @export
dsmatchQTE = function(Y, X, A, p, method = "dsm",
  ...
  if (method == "dsm"){
    ...
    if (!grepl("zir", model.pg)) {
      ...
      for (kk in 1:trtnumber) {
        ...
        if (thistrt == 0) {
          f0 <- function(par) {
            mean(Yiw.ds[, kk] < par) - p -
              sum(pnorm((par - mu0.ds[out1$index.control]) / sqrt(sigsqhat0)) -
                  pnorm((par - mu0.ds[out1$index.treated]) / sqrt(sigsqhat0))) / n
          }
          estq0 <- bisect(f0, lo = min(Yiw.ds[, 1]), hi = max(Yiw.ds[, 1]), ytol = 1e-12, itmax = 100)
        }
        if (thistrt == 1) {
          f1 <- function(par) {
            mean(Yiw.ds[, kk] < par) - p -
              sum(pnorm((par - mu1.ds[out1$index.control]) / sqrt(sigsqhat1)) -
                  pnorm((par - mu1.ds[out1$index.treated]) / sqrt(sigsqhat1))) / n
          }
          estq1 <- bisect(f1, lo = min(Yiw.ds[, 2]), hi = max(Yiw.ds[, 2]), ytol = 1e-12, itmax = 100)
        }
      }
      est.ds <- estq1 - estq0
      ...
      if (varest == 1) {
        if (mc == 1) {
          results <- boot::boot(data = data, statistic = estforboot_dsQTE, R = boots, p = p,
      ...
      for (kk in 1:trtnumber) {
        ...
        if (thistrt == 1) {
          f1 <- function(par) {
            mean(Yiw.ds[, kk] < par) - p -
              sum(pnorm((par - mu1.ds[out1$index.control]) / sqrt(sigsqhat1)) * pb0.ds[out1$index.control] -
                  pnorm((par - mu1.ds[out1$index.treated]) / sqrt(sigsqhat1)) * pb0.ds[out1$index.treated] -
                  (par >= 0) * (pb1.ds[out1$index.control] - pb1.ds[out1$index.treated])) / n
          }
          estq1 <- bisect(f1, lo = min(Yiw.ds[, 2]), hi = max(Yiw.ds[, 2]), ytol = 1e-12, itmax = 100)
        }
      }
    ...
    est.ps <- quantile(Yiw.ps[, 2], p) - quantile(Yiw.ps[, 1], p)
    ...
    if (varest == 1) {
      if (mc == 1) {
        results <- boot::boot(data = data, statistic = estforboot_psQTE, R = boots, p = p,
          model.ps = model.ps, ps = ps, lp.ps = lp.ps,
          parallel = "multicore", ncpus = ncpus)
      }
      if (mc == 0) {
    ...
    for (kk in 1:trtnumber) {
      ...
      if (thistrt == 0) {
        f0 <- function(par) {
          mean(Yiw.pg[, kk] < par) - p -
            sum(pnorm((par - mu0.pg[out1$index.control]) / sqrt(sigsqhat0)) -
                pnorm((par - mu0.pg[out1$index.treated]) / sqrt(sigsqhat0))) / n
        }
        estq0 <- bisect(f0, lo = min(Yiw.pg[,1]), hi = max(Yiw.pg[, 1]), ytol = 1e-12, itmax = 100)
      }
      if (thistrt == 1) {
        f1 <- function(par) {
          mean(Yiw.pg[, kk] < par) - p -
            sum(pnorm((par - mu1.pg[out1$index.control]) / sqrt(sigsqhat1)) -
                pnorm((par - mu1.pg[out1$index.treated]) / sqrt(sigsqhat1))) / n
        }
        estq1 <- bisect(f1, lo = min(Yiw.pg[,2]), hi = max(Yiw.pg[, 2]), ytol = 1e-12, itmax = 100)
      }
    }
    ...
    if (varest == 1) {
      if (mc == 1) {
        results <- boot::boot(data = data, statistic = estforboot_pgQTE, R = boots, p = p,
          model.pg = model.pg, pg = pg, lp.pg = lp.pg,
          parallel = "multicore", ncpus = ncpus)
      }
      if (mc == 0) {
        ...
        f0 <- function(par) {
          mean(Yiw.x[, kk] < par) - p -
            sum(pnorm((par - mu0.x[out1$index.control]) / sqrt(sigsqhat0)) -
                pnorm((par - mu0.x[out1$index.treated]) / sqrt(sigsqhat0))) / n
        }
        estq0 <- bisect(f0, lo = min(Yiw.x[,1]), hi = max(Yiw.x[, 1]), ytol = 1e-12, itmax = 100)
      }
      if (thistrt == 1) {
        f1 <- function(par) {
          mean(Yiw.x[, kk] < par) - p -
            sum(pnorm((par - mu1.x[out1$index.control]) / sqrt(sigsqhat1)) -
                pnorm((par - mu1.x[out1$index.treated]) / sqrt(sigsqhat1))) / n
        }
        estq1 <- bisect(f1, lo = min(Yiw.x[,2]), hi = max(Yiw.x[, 2]), ytol = 1e-12, itmax = 100)
      }
    }
    ...
    if (varest == 1) {
      if (mc == 1) {
        results <- boot::boot(data = data, statistic = estforboot_covQTE, R = boots, p = p,
          parallel = "multicore", ncpus = ncpus)
      }
      if (mc == 0) {
        results <- boot::boot(data = data, statistic = estforboot_covQTE, R = boots, p = p)
