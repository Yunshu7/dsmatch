estforboot_dsATT <- function(data, indices, model.ps, ps = NULL, lp.ps = NULL, model.pg, pg = NULL, lp.pg = NULL){
  data1 <- data[indices, ]
  ...
  boot.ds <- sum(A * (mu1.ds - mu0.ds) + (A - (1 - A) * Kiw.ds) * (Y - mu.ds)) / length(loc.1)
